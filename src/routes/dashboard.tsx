...
function Kpi({
  ...
  return (
    <Card className="relative overflow-hidden border border-border bg-card p-5 hover:border-purple-500/30 transition-all duration-300 group">
      ...
      <p className="mt-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">{label}</p>
      <div className="mt-1 flex items-baseline gap-2">
        <span className="text-3xl font-extrabold tracking-tight text-foreground">{value}</span>
      </div>
      <p className="mt-2 text-xs text-muted-foreground flex items-center justify-between border-t border-border pt-2">
        <span>{hint}</span>
  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl border border-border bg-card p-3 shadow-2xl backdrop-blur-md">
        ...
        {payload[1] && (
          <p className="text-sm font-bold text-cyan-600 dark:text-cyan-400 flex items-center gap-2 mt-0.5">
    ...
    >
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span className="font-semibold text-foreground">Modo Big Data Activo</span>
          <span>·</span>
          <span>{r.total} registros procesados</span>
        </div>
        <div className="flex items-center gap-2.5">
          <div className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground">
          ...
          <button 
            ...
            className="inline-flex items-center gap-1.5 rounded-xl border border-purple-500/30 bg-purple-500/10 px-3.5 py-1.5 text-xs font-semibold text-purple-700 dark:text-purple-300 hover:bg-purple-500/20 transition-all shadow-sm"
          >
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <FadeIn delay={0.1} className="lg:col-span-2">
          <Card className="h-full border border-border bg-card p-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-base font-bold tracking-tight text-foreground">Evolución de Rendimiento y Asistencia</h2>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Seguimiento quincenal de promedios vs índice de concurrencia</p>
              </div>
              <div className="flex items-center gap-4 text-xs">
                <span className="flex items-center gap-1.5 font-medium text-foreground">
                  <span className="h-2 w-2 rounded-full bg-purple-500" /> Promedio
                </span>
                <span className="flex items-center gap-1.5 font-medium text-foreground">
                  <span className="h-2 w-2 rounded-full bg-cyan-500" /> Asistencia
                </span>
              </div>
            </div>
        ...
        <FadeIn delay={0.15} className="lg:col-span-1">
          <Card className="h-full flex flex-col justify-between border border-border bg-card p-6">
            <div>
              <h2 className="text-base font-bold tracking-tight text-foreground">Distribución de Aptitud</h2>
              <p className="text-xs text-muted-foreground mt-0.5">Evaluación automática según reglamento</p>
              ...
              <div className="relative mt-4 flex h-48 items-center justify-center">
                ...
                <div className="pointer-events-none absolute flex flex-col items-center justify-center text-center">
                  <span className="text-2xl font-black tracking-tight text-foreground">{r.pctAptos}%</span>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Aptitud</span>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-2 border-t border-border pt-4">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-foreground">{r.aptos}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-2 text-foreground font-medium">
                    <span className="h-2.5 w-2.5 rounded-full bg-rose-500" /> Inhabilitados
                  </span>
                  <span className="font-bold text-foreground">{r.noAptos}</span>
                </div>
              </div>
            </div>

            <div className="mt-4 rounded-xl border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
              <p className="font-semibold text-foreground">Regla institucional:</p>
      ...
      <FadeIn delay={0.2} className="mt-6">
        <Card className="border border-border bg-card p-6">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold tracking-tight text-foreground">Alumnos que Requieren Intervención</h2>
                <span className="rounded-full bg-rose-500/10 border border-rose-500/20 px-2 py-0.5 text-[10px] font-bold text-rose-600 dark:text-rose-400">
                  Prioridad Alta
                </span>
              </div>
              <p className="text-xs text-muted-foreground mt-0.5">Estudiantes con asistencia crítica por debajo del umbral mínimo</p>
            </div>
            <Link
              to="/alumnos"
              className="inline-flex items-center gap-1 rounded-xl border border-border bg-muted/40 px-3 py-1.5 text-xs font-semibold text-foreground hover:border-purple-500/30 hover:bg-purple-500/10 hover:text-purple-600 dark:hover:text-purple-300 transition-all"
            >
          ...
          <div className="mt-4 divide-y divide-border">
            {estudiantesRiesgo.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className="flex flex-wrap items-center justify-between gap-4 py-3.5 transition-colors hover:bg-muted/30 px-2 rounded-lg"
                >
                  <div className="flex items-center gap-3.5 min-w-[240px]">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-border bg-muted/50 text-xs font-bold text-purple-600 dark:text-purple-400 shadow-inner">
                      {a.nombre
                        .split(" ")
                        .slice(0, 2)
                        .map((n) => n[0])
                    ...
                    <div>
                      <p className="text-sm font-bold text-foreground tracking-tight">{a.nombre}</p>
                      <p className="text-xs text-muted-foreground">{a.curso || "Big Data / Análisis de Datos"}</p>
                    </div>
                  </div>
                  ...
                  <div className="w-48 hidden sm:block">
                    <div className="flex items-center justify-between text-[11px] mb-1">
                      ...
                      <span className={a.asistencia < 70 ? "text-rose-600 dark:text-rose-400 font-bold" : "text-emerald-600 dark:text-emerald-400 font-bold"}>
                        {a.asistencia}%
                      </span>
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-sm font-bold ${a.promedio >= 10.5 ? "text-foreground" : "text-rose-600 dark:text-rose-400"}`}>
                      {a.promedio} <span className="text-[10px] text-muted-foreground font-normal">/20</span>
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link
                      to="/alumnos/$id"
                      params={{ id: a.id }}
                      className="inline-flex h-8 items-center justify-center rounded-lg border border-border bg-muted/40 px-2.5 text-xs font-medium text-foreground hover:border-purple-500/30 hover:bg-purple-500/10 hover:text-purple-600 dark:hover:text-purple-300 transition-all"
                    >
                      Ficha
                    </Link>
                  </div>
