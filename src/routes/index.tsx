...
import { loginWithEmail, loginWithGoogle, sendVerification } from "@/lib/firebase";
...
function Login() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const perfil = await asegurarPerfilUsuario(userCred.user);
      ...
      if (!userCred.user.emailVerified) {
        ...
        setInfoMessage(
          perfil.estado === "pendiente"
            ? "Tu cuenta fue creada y está pendiente de aprobación por el administrador."
            : "Tu acceso fue rechazado por el administrador."
        );
      }
    } catch (err: any) {
  ...
  return (
    <AuthLayout titulo="Iniciar sesión" descripcion="Ingresa con tu usuario aprobado para ver la red academica sincronizada.">
