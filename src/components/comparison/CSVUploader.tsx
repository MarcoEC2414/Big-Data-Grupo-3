...
interface CSVUploaderProps {
  ...
  onDatasetSelected?: (dataset: SharedDataset) => void;
  onPublish?: (csvFile: CSVFile) => Promise<void>;
}
...
export function CSVUploader({
  ...
  return (
    <div className="space-y-4">
      ...
      {!csvFile && datasets.length > 0 && onDatasetSelected && (
        <div className="rounded-lg border border-border bg-card p-4 space-y-2">
          <p className="text-sm font-semibold text-foreground">Datasets compartidos</p>
          <select
            defaultValue=""
            onChange={(event) => {
              const dataset = datasets.find((item) => item.id === event.target.value);
              if (dataset) onDatasetSelected(dataset);
            }}
            className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground"
          >
            <option value="">Selecciona un dataset sincronizado</option>
