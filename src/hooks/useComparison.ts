...
import { escucharDatasetsDisponibles, publicarDataset, type SharedDataset } from '@/lib/comparison/datasetService';
...
export function useComparison() {
  ...
  useEffect(() => {
    // Cargar automáticamente los alumnos de Firestore
    getAlumnosFirebase()
      .then((data) => {
        setFirebaseData(data);
      })
      .catch((err) => {
        console.error("Error al precargar alumnos de Firestore:", err);
      });
    ...
    return escucharDatasetsDisponibles(
      uid,
      (disponibles) => {
        setDatasets(disponibles);
        setDatasetLoading(false);
      },
      () => setDatasetLoading(false)
    );
  ...
  const handleLoadCSV = useCallback(async (csvFile: CSVFile) => {
    try {
      ...
      // Si no hay datos de Firebase aún cargados, cargarlos de inmediato
      if (firebaseData.length === 0) {
        setProgress({ current: 0, total: 0, status: 'loading' });
        const data = await getAlumnosFirebase();
        setFirebaseData(data);
        setProgress({ current: 0, total: 0, status: 'idle' });
      }
    } catch (err: any) {
  ...
  const handlePublishDataset = useCallback(async (csvFile: CSVFile) => {
    const user = auth.currentUser;
    if (!user) throw new Error('Debes iniciar sesión para compartir un dataset.');
    await publicarDataset(csvFile, user.uid, user.email || 'usuario sin correo');
    setCSVFile(csvFile);
  }, []);

  const handleLoadDataset = useCallback((dataset: SharedDataset) => {
    setError(null);
    setCSVFile({
      name: dataset.nombre,
      data: dataset.datos,
      ...
      loadedAt: new Date(),
      size: JSON.stringify(dataset.datos).length,
    });
  }, []);
